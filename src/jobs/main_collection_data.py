...
def transform_data(df, schema_name, data_set, spark):
    try:
        ...
        logger.info(
            "transform_data: Transforming data for table: %s using schema from %s",
            schema_name,
            dataset_json_transformed_path,
        )
        json_data = load_metadata(dataset_json_transformed_path)
        logger.info(
            "transform_data: Transforming data with schema with json data: %s",
            json_data,
        )
        show_df(df, 5, env)
        ...
        if (
            ...
            logger.info(
                "transform_data: Fields to select from json data %s for %s",
                fields,
                schema_name,
            )
        ...
        logger.info(
            "transform_data: DataFrame columns after renaming hyphens: %s", df.columns
        )
        df.printSchema()
            ...
            for table_name in table_names:
                if (
                    ...
                    if table_name == "entity":
                        logger.info(
                            "Main: Invocation of write_to_s3_format method for %s table",
                            table_name,
                        )
                    ...
                    if count is not None:
                        logger.info(
                            "Main: Processed DataFrame for %s table contains %s records",
                            table_name,
                            count,
                        )
