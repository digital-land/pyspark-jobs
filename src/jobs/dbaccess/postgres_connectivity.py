...
def get_aws_secret(environment="development"):
    ...
    try:
        logger.info(
            f"Attempting to retrieve PostgreSQL secrets using EMR-compatible method for environment: {environment}"
        )
...
def cleanup_old_staging_tables(conn_params, max_age_hours=24, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                f"cleanup_old_staging_tables: Starting cleanup (attempt {attempt + 1}/{max_retries})"
            )
            ...
            logger.info(
                f"cleanup_old_staging_tables: Found {len(staging_tables)} staging tables"
            )
                    ...
                    if len(parts) >= 4:
                        ...
                        if (
                            ...
                            if age_hours > max_age_hours:
                                ...
                                dropped_count += 1
                                logger.info(
                                    f"cleanup_old_staging_tables: Dropped old staging table {table_name} (age: {age_hours:.1f}h)"
                                )
                            else:
                                logger.debug(
                                    f"cleanup_old_staging_tables: Keeping recent staging table {table_name} (age: {age_hours:.1f}h)"
                                )
                        else:
                            logger.debug(
                                f"cleanup_old_staging_tables: Skipping table with invalid timestamp format: {table_name}"
                            )
                except Exception as e:
                    logger.warning(
            ...
            conn.commit()
            logger.info(
                f"cleanup_old_staging_tables: Cleanup complete - dropped {dropped_count} old staging tables"
            )
            return
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(
                    f"cleanup_old_staging_tables: Error (attempt {attempt + 1}/{max_retries}): {e}"
                )
                logger.info(
                    f"cleanup_old_staging_tables: Retrying in {retry_delay} seconds..."
                )
                time.sleep(retry_delay)
            else:
                logger.error(
                    f"cleanup_old_staging_tables: All {max_retries} attempts failed: {e}"
                )
        finally:
            if cur:
                try:
        ...
        try:
            ...
            if attempt == 0:  # Only on first attempt
                try:
                    ...
                    logger.warning(
                        f"create_and_prepare_staging_table: Cleanup warning (non-critical): {e}"
                    )
            ...
            logger.debug(
                f"create_and_prepare_staging_table: Creating staging with schema: {column_defs[:200]}..."
            )
            ...
            create_entity_query = (
                f"CREATE TABLE IF NOT EXISTS {dbtable_name} ({entity_column_defs});"
            )
            ...
            logger.info(
                f"create_and_prepare_staging_table: Verified main table '{dbtable_name}' exists"
            )
