...
def get_aws_secret(environment="development"):
    ...
    try:
        logger.info(
            "Attempting to retrieve PostgreSQL secrets using EMR - compatible method for environment: %s",
            environment,
        )

        # Construct secret path based on environment
        secret_path = "/%s-pd-batch/postgres-secret" % environment
        logger.info("Using secret path: %s", secret_path)
        ...
        if missing_fields:
            error_msg = "Missing required secret fields: %s" % missing_fields
            logger.error(error_msg)
            raise ValueError(error_msg)

        logger.info(
            "get_aws_secret: Successfully retrieved database credentials from AWS Secrets Manager"
        )
        ...
        error_msg = "Failed to parse secrets JSON: %s" % e
        logger.error(error_msg)
        raise ValueError(error_msg)
    except Exception as e:
        error_msg = "Failed to retrieve AWS secrets: %s" % e
        logger.error(error_msg)
        raise
...
def cleanup_old_staging_tables(conn_params, max_age_hours=24, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                "cleanup_old_staging_tables: Starting cleanup (attempt %d/%d)",
                attempt + 1,
                max_retries,
            )
            ...
            cur.execute(
                find_staging_query,
                (
                    f"{dbtable_name}_staging_%",
                    f"{dbtable_name}_staging_[a-f0-9]{{8}}_[0-9]{{8}}_[0-9]{{6}}$",
                ),
            )
            ...
            logger.info(
                "cleanup_old_staging_tables: Found %d staging tables",
                len(staging_tables),
            )
            ...
            for table_name in staging_tables:
                try:
                    ...
                    if len(parts) >= 4:
                        ...
                        if (
                            len(timestamp_str) == 15
                            and timestamp_str[8] == "_"
                            and timestamp_str[:8].isdigit()
                            and timestamp_str[9:].isdigit()
                        ):
                            table_time = datetime.strptime(
                                timestamp_str, "%Y%m%d_%H%M%S"
                            )
                            age_hours = (
                                datetime.now() - table_time
                            ).total_seconds() / 3600
                            ...
                            if age_hours > max_age_hours:
                                ...
                                dropped_count += 1
                                logger.info(
                                    "cleanup_old_staging_tables: Dropped old staging table %s (age: %.1fh)",
                                    table_name,
                                    age_hours,
                                )
                            else:
                                logger.debug(
                                    "cleanup_old_staging_tables: Keeping recent staging table %s (age: %.1fh)",
                                    table_name,
                                    age_hours,
                                )
                        else:
                            logger.debug(
                                "cleanup_old_staging_tables: Skipping table with invalid timestamp format: %s",
                                table_name,
                            )
                except Exception as e:
                    logger.warning(
                        f"cleanup_old_staging_tables: Error processing table {table_name}: {e}"
                    )

            conn.commit()
            logger.info(
                "cleanup_old_staging_tables: Cleanup complete - dropped %d old staging tables",
                dropped_count,
            )
            return
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(
                    "cleanup_old_staging_tables: Error (attempt %d/%d): %s",
                    attempt + 1,
                    max_retries,
                    e,
                )
                logger.info(
                    "cleanup_old_staging_tables: Retrying in %d seconds...", retry_delay
                )
                time.sleep(retry_delay)
            else:
                logger.error(
                    "cleanup_old_staging_tables: All %d attempts failed: %s",
                    max_retries,
                    e,
                )
        finally:
            if cur:
                try:
                    ...
                    logger.warning(
                        f"cleanup_old_staging_tables: Error closing cursor: {e}"
                    )
            if conn:
                try:
                    conn.close()
                    ...
                    logger.warning(
                        f"cleanup_old_staging_tables: Error closing connection: {e}"
                    )
            conn = None
            cur = None
...
def create_and_prepare_staging_table(conn_params, dataset_value, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                f"create_and_prepare_staging_table: Creating staging table (attempt {attempt + 1}/{max_retries})"
            )
            ...
            if attempt == 0:  # Only on first attempt
                try:
                    ...
                    logger.warning(
                        "create_and_prepare_staging_table: Cleanup warning "
                        "(non-critical): %s" % e
                    )
            ...
            logger.debug(
                "create_and_prepare_staging_table: Creating staging with "
                "schema: %s..." % column_defs[:200]
            )
            ...
            logger.info(
                f"create_and_prepare_staging_table: Successfully created staging table '{staging_table_name}'"
            )

            # Ensure main entity table exists (with original JSONB columns)
            entity_column_defs = ", ".join(
                [
                    "%s %s" % (col, dtype)
                    for col, dtype in pyspark_entity_columns.items()
                ]
            )
            create_entity_query = "CREATE TABLE IF NOT EXISTS %s (%s);" % (
                dbtable_name,
                entity_column_defs,
            )
            ...
            # Create index on dataset column if it doesn't exist
            logger.info(
                "create_and_prepare_staging_table: Ensuring index on dataset "
                "column for fast deletion"
            )
            create_index_query = (
                "CREATE INDEX IF NOT EXISTS idx_%s_dataset ON %s(dataset);"
                % (dbtable_name, dbtable_name)
            )
            ...
            logger.info(
                "create_and_prepare_staging_table: Verified main table '%s' exists"
                % dbtable_name
            )
            ...
            if attempt < max_retries - 1:
                retry_delay = min(5 * (2**attempt), 30)
                logger.error(
                    f"create_and_prepare_staging_table: Network error (attempt {attempt + 1}/{max_retries}): {e}"
                )
                logger.info(
                    f"create_and_prepare_staging_table: Retrying in {retry_delay} seconds..."
                )
                time.sleep(retry_delay)
            else:
                raise
            ...
            if attempt < max_retries - 1:
                retry_delay = min(5 * (2**attempt), 30)
                logger.error(
                    f"create_and_prepare_staging_table: Error (attempt {attempt + 1}/{max_retries}): {e}",
                    exc_info=True,
                )
                logger.info(
                    f"create_and_prepare_staging_table: Retrying in {retry_delay} seconds..."
                ...
                logger.error(
                    f"create_and_prepare_staging_table: All {max_retries} attempts failed"
                )
                raise
        finally:
            if cur:
                try:
                    ...
                    logger.warning(
                        f"create_and_prepare_staging_table: Error closing cursor: {e}"
                    )
            if conn:
                try:
                    conn.close()
                    ...
                    logger.warning(
                        f"create_and_prepare_staging_table: Error closing connection: {e}"
                    )
            conn = None
            cur = None
...
def commit_staging_to_production(
    conn_params, staging_table_name, dataset_value, max_retries=3
):
    """
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                f"commit_staging_to_production: Starting commit operation (attempt {attempt + 1}/{max_retries})"
            )
            ...
            logger.info(
                f"commit_staging_to_production: Starting transaction for dataset '{dataset_value}'"
            )
            ...
            logger.info(
                f"commit_staging_to_production: Staging table has {staging_count:,} rows"
            )
            ...
            logger.info(
                f"commit_staging_to_production: Deleting existing data for dataset '{dataset_value}'"
            )
            ...
            logger.info(
                f"commit_staging_to_production: Deleted {deleted_count:,} existing rows in {delete_duration:.2f}s"
            )
            ...
            logger.info(
                f"commit_staging_to_production: Inserted {inserted_count:,} rows in {insert_duration:.2f}s"
            )

            # Step 4: Verify row counts match
            if inserted_count != staging_count:
                logger.error(
                    "commit_staging_to_production: Row count mismatch! "
            ...
            try:
                ...
                conn.commit()
                logger.info(
                    f"commit_staging_to_production: Dropped staging table '{staging_table_name}'"
                )
            except Exception as e:
                logger.warning(
                    f"commit_staging_to_production: Failed to drop staging table (non - critical): {e}"
                )
            ...
            logger.error(
                f"commit_staging_to_production: Network error (attempt {attempt + 1}/{max_retries}): {e}"
            )
            ...
            if attempt < max_retries - 1:
                retry_delay = min(5 * (2**attempt), 30)
                logger.info(
                    f"commit_staging_to_production: Retrying in {retry_delay} seconds..."
                )
                time.sleep(retry_delay)
            else:
                raise
            ...
            logger.error(
                f"commit_staging_to_production: Error (attempt {attempt + 1}/{max_retries}): {e}",
                exc_info=True,
            )
            ...
            if cur:
                try:
                    ...
                    logger.warning(
                        f"commit_staging_to_production: Error closing cursor: {e}"
                    )
            if conn:
                try:
                    conn.close()
                    ...
                    logger.warning(
                        f"commit_staging_to_production: Error closing connection: {e}"
                    )
            conn = None
            cur = None
...
def create_table(conn_params, dataset_value, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                f"create_table: Attempting database connection (attempt {attempt + 1}/{max_retries})"
            )
            logger.info(
                f"create_table: Connecting to {conn_params['host']}:{conn_params['port']}"
            )
            ...
            logger.info(
                "create_table: Set statement_timeout to 5 minutes for long operations"
            )
            ...
            if blocking_queries:
                logger.warning(
                    f"create_table: Found {len(blocking_queries)} existing DELETE operations still running from previous attempts"
                )
                for pid, query_start, state, query in blocking_queries:
                    duration = (
                        (datetime.now() - query_start).total_seconds()
                        if query_start
                        else 0
                    )
                    logger.warning(
                        f"create_table: - PID {pid}: running for {duration:.0f}s - {query[:100]}"
                    )
                    # Terminate the stuck query to prevent blocking
                    try:
                        cur.execute(f"SELECT pg_terminate_backend({pid});")
                        logger.info(
                            f"create_table: Terminated stuck DELETE operation (PID {pid})"
                        )
                    except (DatabaseError, InterfaceError) as e:
                        logger.warning(
                            f"create_table: Could not terminate PID {pid}: {e}"
                        )
                conn.commit()
                logger.info(
                    "create_table: Cleaned up stuck DELETE operations, proceeding with fresh attempt"
                )

            # Build CREATE TABLE SQL dynamically
            column_defs = ", ".join(
                [f"{col} {dtype}" for col, dtype in pyspark_entity_columns.items()]
            )
            create_query = f"CREATE TABLE IF NOT EXISTS {dbtable_name} ({column_defs});"
            logger.info(
                f"create_table: Creating table {dbtable_name} with columns: {list(pyspark_entity_columns.keys())}"
            )

            cur.execute(create_query)
            conn.commit()
            logger.info(
                f"create_table: Table '{dbtable_name}' created successfully (if it didn't exist)."
            )
            ...
            if dataset_value:
                ...
                logger.info(
                    f"create_table: Found {record_count} existing records for dataset '{dataset_value}'"
                )
                ...
                if record_count > 0:
                    ...
                    if record_count > 10000:
                        ...
                        logger.info(
                            f"create_table: Large dataset detected ({record_count:,} records). Deleting in batches of {batch_size:,} to avoid locks..."
                        )
                        ...
                        while True:
                            ...
                            if total_deleted > 0:
                                ...
                                logger.info(
                                    f"create_table: Batch {batch_num} completed - deleted {deleted:,} records"
                                )

                            # Check if deletion is complete
                            if deleted < batch_size:
                                logger.info(
                                    f"create_table: Batch deletion complete - last batch had {deleted:,} records in {total_duration:.1f}s total"
                                )
                        ...
                        if not records_exist:
                            logger.info(
                                f"create_table: ✓ Deletion verified - all {total_deleted:,} records successfully deleted for dataset '{dataset_value}' in {total_duration:.1f}s"
                            )
                        else:
                            ...
                            logger.warning(
                                f"create_table: ⚠ Verification failed - {remaining_count:,} records still remain for dataset '{dataset_value}' (deleted {total_deleted:,})"
                            )

                            # Try one more batch deletion for remaining records
                            logger.info(
                                f"create_table: Attempting to delete remaining {remaining_count:,} records..."
                            )
                            delete_query = (
                                f"DELETE FROM {dbtable_name} WHERE dataset = %s;"
                            )
                            cur.execute(delete_query, (dataset_value,))
                            final_deleted = cur.rowcount
                            conn.commit()
                            logger.info(
                                f"create_table: Final cleanup deleted {final_deleted:,} remaining records"
                            )

                    else:
                        # Small dataset - delete all at once
                        logger.info(
                            f"create_table: Deleting {record_count} records from {dbtable_name} where dataset = {dataset_value}"
                        )
                        delete_query = f"DELETE FROM {dbtable_name} WHERE dataset = %s;"
                        ...
                        conn.commit()
                        logger.info(
                            f"create_table: DELETE operation completed - {deleted_count} rows affected"
                        )
                        ...
                        if remaining_count == 0:
                            logger.info(
                                f"create_table: ✓ Deletion verified - all {deleted_count} records successfully deleted for dataset '{dataset_value}'"
                            )
                        else:
                            logger.error(
                                f"create_table: ✗ Deletion verification failed - {remaining_count} records still exist after DELETE (expected 0)"
                            )
                            raise DatabaseError(
                                f"DELETE operation failed to remove all records for dataset '{dataset_value}' - {remaining_count} records remaining"
                            )
                else:
                    logger.info(
                        f"create_table: No existing records to delete for dataset '{dataset_value}'"
                    )
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(
                    f"create_table: Network / Interface error (attempt {attempt + 1}/{max_retries}): {e}"
                )
                ...
                if "network error" in error_str or "timeout" in error_str:
                    ...
                    logger.error(
                        "create_table: 1. EMR Serverless not configured in correct VPC"
                    )
                    logger.error(
                        "create_table: 2. Security group rules blocking connection"
                    )
                    logger.error(
                        "create_table: 3. RDS database not accessible from EMR subnet"
                    )

                logger.info(
                    f"create_table: Retrying in {retry_delay} seconds with exponential backoff..."
                )
                time.sleep(retry_delay)
            else:
                logger.error(
                    f"create_table: All {max_retries} connection attempts failed"
                )
                raise
            ...
            if is_retryable and attempt < max_retries - 1:
                retry_delay = min(5 * (2**attempt), 60)
                logger.error(
                    f"create_table: Retryable database error (attempt {attempt + 1}/{max_retries}): {e}"
                )
            ...
            if attempt < max_retries - 1:
                ...
                retry_delay = min(5 * (2**attempt), 60)
                logger.error(
                    f"create_table: Unexpected error (attempt {attempt + 1}/{max_retries}): {e}",
                    exc_info=True,
                )
                logger.info(
                    f"create_table: Retrying in {retry_delay} seconds with exponential backoff..."
                ...
                logger.error(
                    f"create_table: All {max_retries} attempts failed with unexpected errors"
                )
                raise
        finally:
            if cur:
...
def _prepare_geometry_columns(df):
    ...
    logger.info(
        "_prepare_geometry_columns: Processing DataFrame for PostgreSQL compatibility"
    )
    ...
    for geom_col in geometry_columns:
        if geom_col in df.columns:
            logger.info(
                f"_prepare_geometry_columns: Processing geometry column: {geom_col}"
            )
            processed_df = processed_df.withColumn(
                ...
                .when(
                    col(geom_col).startswith("POINT"),
                    expr(f"concat('SRID=4326;', {geom_col})"),
                )
                .when(
                    col(geom_col).startswith("POLYGON"),
                    expr(f"concat('SRID=4326;', {geom_col})"),
                )
                .when(
                    col(geom_col).startswith("MULTIPOLYGON"),
                    expr(f"concat('SRID=4326;', {geom_col})"),
                )
...
def calculate_centroid_wkt(conn_params, target_table=None, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                "calculate_centroid_wkt: Connecting to database to calculate "
            ...
            logger.info(
                f"calculate_centroid_wkt: Updated {rows_updated} rows with calculated centroids"
            )
            return rows_updated
            ...
            if attempt < max_retries - 1:
                retry_delay = min(5 * (2**attempt), 30)
                logger.error(
                    f"calculate_centroid_wkt: Network error (attempt {attempt + 1}/{max_retries}): {e}"
                )
                logger.info(
                    f"calculate_centroid_wkt: Retrying in {retry_delay} seconds..."
                )
                time.sleep(retry_delay)
            else:
                raise
            ...
            if attempt < max_retries - 1:
                retry_delay = min(5 * (2**attempt), 30)
                logger.error(
                    f"calculate_centroid_wkt: Error (attempt {attempt + 1}/{max_retries}): {e}"
                )
                logger.info(
                    f"calculate_centroid_wkt: Retrying in {retry_delay} seconds..."
                )
            ...
            if conn:
                try:
                    ...
                    logger.warning(
                        f"calculate_centroid_wkt: Error closing connection: {e}"
                    )
            conn = None
            cur = None
...
def write_to_postgres(
    ...
    **kwargs,
    ...
    return _write_to_postgres_optimized(
        df, dataset, conn_params, batch_size, num_partitions, target_table
    )
...
#         logger.info(
#             "_write_to_postgres_standard: Inserted %d rows into %s using standard JDBC",
#             processed_df.count(), dbtable_name
#         )
#     except Exception as e:
...
def _write_to_postgres_optimized(
    df,
    dataset,
    conn_params,
    batch_size=None,
    ...
    logger.info(
        f"_write_to_postgres_optimized: Starting optimized PostgreSQL write to table '{table_name}'"
    )
    ...
    if not target_table:
        ...
        logger.info(
            f"_write_to_postgres_optimized: Writing to staging table '{target_table}', skipping create_table"
        )
    ...
    if batch_size is None:
        ...
        logger.info(
            f"_write_to_postgres_optimized: Auto - calculated batch size: {batch_size} for {row_count} rows"
        )
    ...
    if num_partitions is None:
        ...
        logger.info(
            f"_write_to_postgres_optimized: Auto - calculated partitions: {num_partitions} for {row_count} rows"
        )

    # Repartition DataFrame for optimal parallel writing
    if df.rdd.getNumPartitions() != num_partitions:
        logger.info(
            f"_write_to_postgres_optimized: Repartitioning DataFrame from {df.rdd.getNumPartitions()} to {num_partitions} partitions"
        )
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                f"_write_to_postgres_optimized: Write attempt {attempt + 1}/{max_retries}"
            )
            logger.info(
                f"_write_to_postgres_optimized: Writing {processed_df.count()} rows to PostgreSQL with batch size {batch_size}"
            )
            logger.info(
                f"_write_to_postgres_optimized: Using {num_partitions} partitions for parallel writes"
            )

            # Use optimized JDBC write with error handling
            processed_df.write.mode("append").option(
                "numPartitions", num_partitions
            ).jdbc(url=url, table=table_name, properties=properties)

            logger.info(
                f"_write_to_postgres_optimized: Successfully wrote data to {table_name} using optimized JDBC writer"
            )
            return  # Success - exit function
            ...
            if is_retryable and attempt < max_retries - 1:
                ...
                retry_delay = min(5 * (2**attempt), 60)
                logger.warning(
                    f"_write_to_postgres_optimized: Write failed (attempt {attempt + 1}/{max_retries}): {e}"
                )
                logger.warning(
                    f"_write_to_postgres_optimized: Aurora may be busy - retrying in {retry_delay} seconds..."
                )
                logger.warning(
                    f"_write_to_postgres_optimized: Error type: {type(e).__name__}"
                )
                time.sleep(retry_delay)
            else:
                # Non - retryable error or final attempt - log and raise
                logger.error(
                    f"_write_to_postgres_optimized: Failed to write to PostgreSQL after {attempt + 1} attempts: {e}"
                )
                logger.error("_write_to_postgres_optimized: Troubleshooting steps:")
                logger.error(
                    "_write_to_postgres_optimized: 1. Check if PostgreSQL JDBC driver is available"
                )
                logger.error(
                    "_write_to_postgres_optimized: 2. Verify network connectivity to database"
                )
                logger.error(
                    "_write_to_postgres_optimized: 3. Check database permissions for the user"
                )
                logger.error(
                    "_write_to_postgres_optimized: 4. Verify table schema matches DataFrame columns"
                )
                logger.error(
                    "_write_to_postgres_optimized: 5. Check if Aurora instance is busy or at capacity"
                )
                raise
        ...
        recommendations.update(
            {
                ...
                "num_partitions": 2,
                "notes": [
                    "Small - medium dataset - conservative batch size for reliability"
                ],
            }
        )
        ...
        recommendations.update(
            {
                ...
                "num_partitions": 8,
                "notes": [
                    "Large dataset - conservative batch processing for stability"
                ],
            }
        )
