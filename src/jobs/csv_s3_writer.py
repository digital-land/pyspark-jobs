...
def _write_single_csv_file(
    ...
    try:
        ...
        logger.info(
            f"_write_single_csv_file: CSV written to temporary location: {temp_path}"
        )
        ...
        logger.info(
            f"_write_single_csv_file: Successfully created CSV file: {final_path}"
        )
        return final_path
...
def _move_csv_to_final_location(temp_path: str, final_csv_path: str) -> str:
    ...
    try:
        ...
        if file_size > 5 * 1024 * 1024 * 1024:  # 5GB
            logger.info(
                f"_move_csv_to_final_location: Large file detected ({file_size} bytes), using multipart copy"
            )
            ...
            logger.info(
                f"_move_csv_to_final_location: Regular file size ({file_size} bytes), using standard copy"
            )
...
def cleanup_temp_csv_files(csv_path: str):
    ...
    try:
        ...
        if csv_path.endswith(".csv"):
            ...
            if "Contents" in response:
                ...
                if objects_to_delete:
                    ...
                    logger.info(
                        f"cleanup_temp_csv_files: Deleted {len(objects_to_delete)} temporary files"
                    )
                else:
                    logger.info(
        ...
        logger.warning(
            f"cleanup_temp_csv_files: Failed to cleanup temporary CSV files: {e}"
        )
        logger.warning(
