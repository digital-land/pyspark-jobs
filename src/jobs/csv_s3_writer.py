...
def prepare_dataframe_for_csv(df):
    ...
    for col_name, col_type in columns_info:
        logger.debug(
            f"prepare_dataframe_for_csv: Processing column {col_name} of type {col_type}"
        )
        ...
        if (
            "struct" in col_type
            or "map" in col_type
            or (col_name.lower() in ["geojson", "json"] and "string" not in col_type)
        ):
            logger.info(
                f"prepare_dataframe_for_csv: Converting {col_name} to JSON string"
            )
        ...
        # Handle date / timestamp columns - convert to standardized format
            ...
            logger.info(
                f"prepare_dataframe_for_csv: Converting {col_name} to standardized date format"
            )
            if "timestamp" in col_type:
                ...
                processed_df = processed_df.withColumn(
                    ...
                    .otherwise(
                        date_format(col(col_name), CSV_CONFIG["timestamp_format"])
                    ),
                )
            else:
...
def write_dataframe_to_csv_s3(
    ...
    try:
        ...
        if cleanup_existing:
            ...
            logger.info(
                f"write_dataframe_to_csv_s3: Cleaned up {cleanup_summary['objects_deleted']} objects"
            )
            if cleanup_summary["errors"]:
                logger.warning(
                    f"write_dataframe_to_csv_s3: Cleanup warnings: {cleanup_summary['errors']}"
                )
...
def _write_single_csv_file(
    ...
    final_csv_path = (
        f"{output_path.rstrip('/')}/{table_name}_{dataset_name}_{timestamp}.csv"
    )
    ...
    try:
        ...
        logger.info(
            "_write_single_csv_file: CSV written to temporary location: %s", temp_path
        )
        ...
        logger.info(
            "_write_single_csv_file: Successfully created CSV file: %s", final_path
        )
        return final_path
...
def _write_multiple_csv_files(
    df, output_path: str, table_name: str, dataset_name: str, config: Dict
) -> str:
    ...
    try:
        ...
        # Write CSV files
        writer = (
            partitioned_df.write.format("csv")
            .option("header", str(config["include_header"]).lower())
            .option("sep", config["sep"])
            .option("quote", config["quote_char"])
            .option("escape", config["escape_char"])
        ...
        logger.info(
            f"_write_multiple_csv_files: Successfully created CSV files in: {csv_dir_path}"
        )
        return csv_dir_path

    except Exception as e:
        logger.error(
            f"_write_multiple_csv_files: Failed to write multiple CSV files: {e}"
        )
        raise
...
def _move_csv_to_final_location(temp_path: str, final_csv_path: str) -> str:
    ...
    try:
        ...
        if file_size > 5 * 1024 * 1024 * 1024:  # 5GB
            logger.info(
                "_move_csv_to_final_location: Large file detected (%s bytes), using multipart copy",
                file_size,
            )
            ...
            future = transfer.copy(copy_source, bucket, final_key, extra_args={})
            future.result()  # Wait for completion

        else:
            # Use regular copy for smaller files
            logger.info(
                "_move_csv_to_final_location: Regular file size (%s bytes), using standard copy",
                file_size,
            )
        ...
        logger.info(
            f"_move_csv_to_final_location: Moved CSV to final location: {final_csv_path}"
        )
...
def cleanup_temp_csv_files(csv_path: str):
    ...
    try:
        logger.info(
            "cleanup_temp_csv_files: Cleaning up temporary CSV files at: %s", csv_path
        )
        ...
        if csv_path.endswith(".csv"):
            ...
            logger.info("cleanup_temp_csv_files: Deleted CSV file: %s", csv_path)
        else:
            ...
            if "Contents" in response:
                ...
                if objects_to_delete:
                    ...
                    logger.info(
                        "cleanup_temp_csv_files: Deleted %s temporary files",
                        len(objects_to_delete),
                    )
                else:
                    logger.info(
                        "cleanup_temp_csv_files: No temporary files found to clean up"
                    )
            else:
                logger.info(
                    "cleanup_temp_csv_files: No temporary files found to clean up"
        ...
        logger.info(
            "cleanup_temp_csv_files: Temporary CSV cleanup completed successfully"
        )

    except Exception as e:
        # Log warning but don't fail the import process due to cleanup issues
        logger.warning(
            "cleanup_temp_csv_files: Failed to cleanup temporary CSV files: %s", e
        )
        logger.warning(
            "cleanup_temp_csv_files: Import was successful, but manual cleanup may be needed"
        )
...
@log_execution_time
def read_csv_from_s3(spark, csv_path: str, infer_schema: bool = True):
    """
    ...
    try:
        ...
        row_count = df.count()
        logger.info(
            f"read_csv_from_s3: Successfully loaded {row_count} rows from {csv_path}"
        )
...
def get_aurora_connection_params(env: str) -> Dict[str, str]:
    ...
    try:
        logger.info(
            "get_aurora_connection_params: Retrieving Aurora connection parameters"
        )
        ...
        logger.info(
            "get_aurora_connection_params: Successfully retrieved connection parameters"
        )
        return conn_params

    except Exception as e:
        logger.error(
            f"get_aurora_connection_params: Failed to get connection params: {e}"
        )
...
def import_csv_to_aurora(
    ...
    try:
        if use_s3_import:
            ...
            jdbc_result = _import_via_jdbc(
                csv_s3_path, table_name, dataset_name, env, truncate_table
            )
        ...
        logger.info(
            "import_csv_to_aurora: Cleaning up temporary CSV files after import failure"
        )
        cleanup_temp_csv_files(csv_s3_path)
...
def _import_via_aurora_s3(
    csv_s3_path: str, table_name: str, dataset_name: str, truncate_table: bool, env: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        if truncate_table:
            logger.info(
                f"_import_via_aurora_s3: Truncating table {table_name} for dataset {dataset_name}"
            )
            cursor.execute(
                f"DELETE FROM {table_name} WHERE dataset = %s;", (dataset_name,)
            )
            deleted_rows = cursor.rowcount
            conn.commit()
            logger.info(
                f"_import_via_aurora_s3: Deleted {deleted_rows} existing rows for dataset {dataset_name}"
            )
        ...
        # Execute S3 import
        logger.info(
            f"_import_via_aurora_s3: Importing CSV from s3://{bucket_name}/{object_key}"
        )
        cursor.execute(
            import_sql,
            (
        ...
        logger.info(
            f"_import_via_aurora_s3: Successfully imported {rows_imported} rows"
        )
...
def _import_via_jdbc(
    csv_s3_path: str, table_name: str, dataset_name: str, env: str, truncate_table: bool
) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"_import_via_jdbc: Successfully imported {row_count} rows via JDBC"
            )
...
Examples:
  ...
  python csv_s3_writer.py --input s3://bucket/parquet/ \
    --output s3://bucket/csv/ --table entity --dataset my-dataset

  # Import existing CSV to Aurora using S3 import
  python csv_s3_writer.py --import-csv s3://bucket/csv/entity.csv \
    --table entity --dataset my-dataset

  # Import using JDBC fallback
  python csv_s3_writer.py --import-csv s3://bucket/csv/entity.csv \
    --table entity --dataset my-dataset --use-jdbc
        """,
    )

    parser.add_argument(
        "--input", "-i", help="Input path to parquet files to convert to CSV"
    )
