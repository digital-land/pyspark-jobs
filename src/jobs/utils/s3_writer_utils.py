...
def write_to_s3_format(df, output_path, dataset_name, table_name, spark, env):
    try:
        ...
        temp_output_path = (
            f"s3://{env}-collection - data / dataset / temp/{dataset_name}/"
        )
        ...
        if count is not None:
            logger.info(
                "write_to_s3_format: Input DataFrame After schema fields ensured and "
                "before writing csv for %s table contains %d records",
                table_name,
                count,
            )
