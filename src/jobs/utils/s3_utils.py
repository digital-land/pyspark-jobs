...
def parse_s3_path(s3_path: str) -> Tuple[str, str]:
    ...
    if '/' not in clean_path:
        raise S3UtilsError(f"Invalid S3 path format: {s3_path}. Expected format: s3://bucket/prefix")
...
def cleanup_dataset_data(output_path: str, dataset_name: str) -> dict:
    ...
    try:
        ...
        logger.info(f"cleanup_dataset_data: Checking for existing data in s3://{bucket}/{dataset_prefix}")
                    ...
                    for i in range(0, len(objects_to_delete), 1000):
                        ...
                        try:
                            ...
                            if 'Deleted' in response:
                                cleanup_summary['objects_deleted'] += len(response['Deleted'])
        ...
        if error_code == 'NoSuchBucket':
            ...
            logger.warning(f"cleanup_dataset_data: {error_msg}, proceeding with write operation")
            cleanup_summary['errors'].append(error_msg)
            return cleanup_summary
...
def validate_s3_bucket_access(bucket_name: str) -> bool:
    ...
    try:
        ...
        error_code = e.response.get('Error', {}).get('Code', 'Unknown')
        logger.warning(f"validate_s3_bucket_access: Bucket '{bucket_name}' validation failed: {error_code}")
        return False
