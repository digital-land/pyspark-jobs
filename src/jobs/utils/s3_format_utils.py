...
# Function to recursively flatten the DataFrame
    ...
    while nested_cols:
        ...
        expanded = [col(f"{col_to_flatten}.{k}").alias(f"{col_to_flatten}_{k}") 
                    for k in nested_df.select(col_to_flatten + ".*").columns]
        nested_df = nested_df.select("*", *expanded).drop(col_to_flatten)
        nested_cols = [c[0] for c in nested_df.dtypes if c[1].startswith('struct')]

    return nested_df.select(*flat_cols, *[c for c in nested_df.columns if c not in flat_cols])
...
def flatten_s3_geojson(df):
    df = df.withColumn("longitude", regexp_extract("point", r"POINT \(([^ ]+)", 1).cast("double"))
    df = df.withColumn("latitude", regexp_extract("point", r"POINT \([^ ]+ ([^\)]+)\)", 1).cast("double"))
    ...
    properties_expr = create_map(*[lit(col_name) for col in property_cols for col_name in [col, col]])
