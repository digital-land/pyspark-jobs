...
def transform_data_fact(df):
    try:
        ...
        logger.info(
            "transform_data_fact:Window specification defined for partitioning by 'fact' and ordering %s",
            window_spec,
        )
        # Add row number
        df_with_rownum = df.withColumn("row_num", row_number().over(window_spec))
        logger.info(
            "transform_data_fact:Row number added to DataFrame %s",
            df_with_rownum.columns,
        )
        ...
        logger.info(
            "transform_data_fact:Final DataFrame after filtering: %s", transf_df.columns
        )
        transf_df = transf_df.select(
            "end_date",
            "entity",
...
def transform_data_fact_res(df):
    try:
        ...
        logger.info(
            "transform_data_fact_res:Final DataFrame after filtering: %s",
            transf_df.columns,
        )
...
def transform_data_entity(df, data_set, spark, env):
    try:
        ...
        logger.info(
            "transform_data_entity: Fetched typology value from dataset specification for dataset: %s is %s",
            data_set,
            typology_value,
        )
        ...
        logger.info(
            "transform_data_entity: Setting dataset column to %s and dropping geojson column if exists",
            data_set,
        )
