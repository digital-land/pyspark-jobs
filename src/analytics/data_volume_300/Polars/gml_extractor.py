...
class GMLFileExtractor:
    def __init__(self, downloads_dir=None, extracted_dir=None):
        ...
        self.downloads_dir = Path(downloads_dir) if downloads_dir else base_dir / "downloads"
        self.extracted_dir = Path(extracted_dir) if extracted_dir else Path(__file__).parent / "extracted_gml"
    ...
    def extract_all(self, max_workers=None):
        ...
        self.logger.info(f"Found {len(zip_files)} zip files to extract with {max_workers} workers")
        
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(self._extract_gml_static, zip_file, self.extracted_dir): zip_file 
