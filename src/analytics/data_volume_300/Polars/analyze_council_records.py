...
def analyze_parquet_data(parquet_dir):
    ...
    ).sort('record_count', descending=True).collect()
    ...
    print(f"Max Records: {result['record_count'].max():,} ({result.filter(pl.col('record_count') == result['record_count'].max())['council_name'][0]})")
    print(f"Min Records: {result['record_count'].min():,} ({result.filter(pl.col('record_count') == result['record_count'].min())['council_name'][0]})")
