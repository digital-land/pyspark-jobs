...
if __name__ == "__main__":
    ...
    print(f"Total .gml data volume: {total_gml_size:,} bytes ({format_size(total_gml_size)})")
    print(f"{'='*60}")
    ...
    with open(output_file, 'w') as f:
        ...
        for detail in details:
            ...
            f.write(f"  Size: {detail['gml_size']:,} bytes ({format_size(detail['gml_size'])})\n")
            if detail['gml_files']:
        ...
        f.write(f"Total data volume: {total_gml_size:,} bytes ({format_size(total_gml_size)})\n")
