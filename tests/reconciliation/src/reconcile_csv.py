...
with open(output, 'w') as f:
    ...
    if point_distances:
        ...
        f.write(f"  Avg distance: {sum(point_distances)/len(point_distances):.10f} degrees\n")
        f.write(f"  Significant differences (>={SIGNIFICANT_DISTANCE_THRESHOLD}): {field_mismatch_counts.get('point', 0)}\n")
        f.write(f"\n  Note: A significant difference is defined as a distance >= {SIGNIFICANT_DISTANCE_THRESHOLD} degrees\n")
        f.write(f"  (~11 meters). Differences below this threshold are considered negligible and\n")
        f.write(f"  likely due to floating-point precision variations during data processing.\n")
...
if significant_point_diffs:
    ...
    with open(sig_output, 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=['entity', 'distance_degrees', 'distance_meters', 'point_file1', 'point_file2'])
        writer.writeheader()
        writer.writerows(significant_point_diffs)
    print(f"Reconciliation complete. Report saved to: {output}")
    print(f"Significant point differences saved to: {sig_output} ({len(significant_point_diffs)} records)")
else:
    print(f"Reconciliation complete. Report saved to: {output}")
