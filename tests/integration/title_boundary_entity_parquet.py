...
def spark():
    ...
    return (
        SparkSession.builder.appName("ParquetTestSuite")
        .master("local[2]")
...
def test_parquet_file_partitioning(spark, sample_parquet_data):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        sample_parquet_data.write.mode("overwrite").partitionBy("entry_date").parquet(
            partitioned_path
        )
...
def test_parquet_file_compression(spark, sample_parquet_data):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        sample_parquet_data.write.mode("overwrite").option(
            "compression", "snappy"
        ).parquet(snappy_path)
