"""
Integration tests for logging configuration to diagnose logging issues.
    ...
    try:
        ...
        if os.path.exists(log_file):
            with open(log_file, 'r') as f:
                content = f.read()
                if "Debug message to file" in content and "Error message to file" in content:
                    ...
                    print(f"❌ Log file created but missing expected content")
...
def test_logger_config_executable():
    ...
    try:
        ...
        module_path = os.path.join(os.path.dirname(__file__), '..', '..', 'src', 'jobs', 'utils', 'logger_config.py')
        ...
        if result.returncode == 0:
            ...
            print(f"❌ Logger config module execution failed with return code {result.returncode}")
            print("STDOUT:")
            print(result.stdout)
            print("STDERR:")
            print(result.stderr)
