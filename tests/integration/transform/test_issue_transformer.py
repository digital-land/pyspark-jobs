"""
Integration tests for transform_issue.
...
def test_transform_issue_adds_missing_fields_as_null(spark):
    ...
    result = transform_issue(df, "ds-a")
...
def test_transform_issue_removes_null_resource_rows(spark):
...
def test_transform_issue_selects_schema_fields_in_order(spark):
    ...
    result = transform_issue(df, "ds-a")
    expected = [f.field for f in get_schema("issue").fields] + ["processed_timestamp"]

    assert result.columns == expected


def test_transform_issue_preserves_existing_field_values(spark):
