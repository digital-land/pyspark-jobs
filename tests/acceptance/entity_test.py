...
def test_complete_etl_pipeline_integration(spark, temp_csv_files):
    ...
    with patch("jobs.transform_collection_data.spark.read") as mock_spark_read:
        ...
        mock_org_df = spark.read.option("header", True).csv(
            temp_csv_files["organisation_csv"]
        )
        mock_dataset_df = spark.read.option("header", True).csv(
            temp_csv_files["dataset_csv"]
        )
        ...
        processed_df = (
            raw_df.groupBy("entry_number", "entity").pivot("field").agg(first("value"))
        )
