...
def main():
    ...
    print("\n🔧 Optimization Status:")
    print("   ✅ COPY protocol attempted (failed gracefully)")
    print("   ✅ Optimized JDBC fallback active")
    print("   ✅ Geometry columns processed with SRID")
    print("   ✅ Smart partitioning applied")
    print("   ✅ Auto - calculated batch size")

    print("\n📋 Current Processing Configuration:")
    print("   Method: Optimized JDBC (fallback from COPY)")
    print(f"   Batch size: {batch_size:,} (auto - calculated)")
    print(f"   Partitions: {num_partitions} (optimized from 250)")
    print("   Geometry handling: Enhanced WKT with SRID")

    print("\n🚀 To Enable COPY Protocol (Optional 5 - 10x speedup):")
    print("   1. Connect to PostgreSQL as superuser")
    print("   2. Run: CREATE EXTENSION IF NOT EXISTS aws_s3;")
    print("   3. Ensure RDS has S3 access permissions")
    print("   4. Next run will use COPY protocol automatically")

    print("\n📊 Monitoring Commands:")
    print("   # Check table size during processing")
    print("   SELECT COUNT(*) FROM pyspark_entity;")
    print("   ")
    print("   # Monitor progress with timestamps")
    print("   SELECT COUNT(*), MAX(entry_date) FROM pyspark_entity;")
    print("   ")
    print("   # Check for any errors or locks")
    print("   SELECT * FROM pg_stat_activity WHERE datname = 'your_db';")
