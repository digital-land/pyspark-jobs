...
class TestS3WriterUtilsAdditional:
    ...
    def test_write_to_s3_format_full_pipeline(self, mock_cleanup_temp, mock_calculate_centroid, mock_flatten_json, mock_boto3):
        ...
        mock_df.toLocalIterator.return_value = [
            Mock(asDict=Mock(return_value={'entity': '1', 'name': 'test', 'geometry': 'POINT (1 2)'}))
        ]
        ...
        with patch('jobs.utils.s3_writer_utils.read_csv_from_s3') as mock_read_csv, \
             ...
             patch('jobs.utils.s3_writer_utils.count_df') as mock_count_df, \
             patch('jobs.utils.s3_writer_utils.show_df') as mock_show_df, \
             patch('jobs.utils.s3_writer_utils.ensure_schema_fields') as mock_ensure_schema, \
            ...
            try:
                result = write_to_s3_format(mock_df, "s3://test-bucket/output/", "test-dataset", "entity", mock_spark, "development")
    ...
    def test_transform_data_entity_format_no_priority_column(self):
        ...
        with patch('jobs.utils.s3_writer_utils.get_dataset_typology') as mock_get_typology, \
             patch('jobs.utils.s3_writer_utils.show_df') as mock_show_df:
            
            mock_get_typology.return_value = "test-typology"
            
            try:
                result = transform_data_entity_format(mock_df, "test-dataset", mock_spark, "development")
    ...
    def test_transform_data_entity_format_missing_columns(self):
        ...
        mock_df.groupBy.return_value = mock_df
        mock_pivot_df = Mock()
        mock_pivot_df.columns = ["entity", "name"]  # Missing geometry, end_date, start_date, point
    ...
    def test_normalise_dataframe_schema_issue_table(self):
        ...
        with patch('jobs.main_collection_data.load_metadata') as mock_load_metadata:
            ...
            with pytest.raises(ValueError, match="Unknown table name"):
                ...
                normalise_dataframe_schema(mock_df, "issue", "test-dataset", mock_spark, "development")
    ...
    def test_normalise_dataframe_schema_fact_table(self):
        ...
        with patch('jobs.main_collection_data.load_metadata') as mock_load_metadata:
            ...
            with pytest.raises(ValueError, match="Unknown table name"):
                ...
                normalise_dataframe_schema(mock_df, "fact", "test-dataset", mock_spark, "development")
...
"""
    ...
    def test_s3_rename_and_move_existing_file_deletion(self, mock_boto3):
        ...
        mock_s3_client.list_objects_v2.return_value = {
            'Contents': [
                {'Key': 'dataset/temp/test-dataset/part-00000.json'}
            ]
        }
    ...
    def test_write_to_s3_error_handling(self):
        ...
        with patch('jobs.utils.s3_writer_utils.cleanup_dataset_data') as mock_cleanup:
            ...
            with pytest.raises(Exception, match="Spark error"):
                write_to_s3(mock_df, "s3://test-bucket/output/", "test-dataset", "entity", "development")
