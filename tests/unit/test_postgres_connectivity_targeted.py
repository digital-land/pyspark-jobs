...
class TestPostgresConnectivityTargeted:
    ...
    def test_create_and_prepare_staging_table_success(self, mock_pg8000):
        ...
        with patch.dict('sys.modules', {'pg8000': Mock(), 'pg8000.exceptions': Mock()}):
            ...
            with patch('jobs.dbaccess.postgres_connectivity.cleanup_old_staging_tables') as mock_cleanup:
                try:
    ...
    def test_commit_staging_to_production_success(self, mock_pg8000):
        ...
        with patch.dict('sys.modules', {'pg8000': Mock(), 'pg8000.exceptions': Mock()}):
            ...
            # Mock row counts
            mock_cur.fetchone.side_effect = [(1000,), (500,)]  # staging count, then verification
            mock_cur.rowcount = 1000  # rows affected by operations
            
            conn_params = {"host": "localhost", "port": 5432, "database": "test"}
            
            try:
                result = commit_staging_to_production(conn_params, "test_staging_table", "test-dataset")
    ...
    def test_create_table_large_dataset_batching(self, mock_pg8000):
        ...
        with patch.dict('sys.modules', {'pg8000': Mock(), 'pg8000.exceptions': Mock()}):
            ...
            mock_cur.fetchone.side_effect = [(50000,), (25000,), (0,)]  # count, batch result, verification
    ...
    def test_write_to_postgres_optimized_flow(self, mock_prepare_geom, mock_create_table):
        ...
        with patch.dict('sys.modules', {'pg8000': Mock(), 'pg8000.exceptions': Mock()}):
            ...
            conn_params = {"host": "localhost", "port": 5432, "database": "test", "user": "test", "password": "test"}
    ...
    def test_prepare_geometry_columns_operations(self):
        ...
        with patch.dict('sys.modules', {'pg8000': Mock(), 'pg8000.exceptions': Mock()}):
            ...
            assert mock_df.withColumn.call_count >= 2  # entity cast + geometry processing
