...
class TestHighCoverageModules:
    ...
    def test_csv_s3_writer_missing_lines(self):
        ...
        with patch.dict('sys.modules', {'pyspark.sql': Mock(), 'pyspark.sql.functions': Mock(), 'boto3': Mock()}):
            from jobs.csv_s3_writer import prepare_dataframe_for_csv, get_aurora_connection_params
            
            # Test line 146 - struct column handling
            mock_df = Mock()
            mock_df.schema.fields = [Mock(name="json_col", dataType=Mock(__str__=lambda x: "struct<field:string>"))]
            ...
            with patch('jobs.csv_s3_writer.get_secret_emr_compatible') as mock_secret:
                mock_secret.return_value = '{"host": "localhost"}'  # Missing required fields
                try:
