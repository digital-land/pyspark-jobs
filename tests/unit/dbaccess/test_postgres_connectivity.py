...
class TestPostgresConnectivity:
    ...
    def test_create_and_prepare_staging_table_no_pg8000(self, caplog):
        ...
        with pytest.raises(
            ImportError, match="pg8000 required for direct database connections"
        ):
    ...
    @pytest.mark.parametrize(
        "staging_count,insert_count,expected_success,expected_error",
        [
    ...
    @patch("jobs.dbaccess.postgres_connectivity.pg8000")
    def test_commit_staging_to_production(
        self, mock_pg8000, staging_count, insert_count, expected_success, expected_error
    ):
    ...
    @pytest.mark.parametrize(
        "row_count,expected_batch,expected_partitions",
        [
            (5000, 1000, 1),  # Small
    ...
    def test_get_performance_recommendations(
        self, row_count, expected_batch, expected_partitions
    ):
