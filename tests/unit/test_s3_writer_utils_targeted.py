...
"""
    ...
    def test_write_to_s3_basic_flow(self, mock_count_df, mock_show_df, mock_cleanup):
        ...
        try:
            ...
            assert mock_df.withColumn.call_count >= 4  # dataset, date parsing, year, month, day, timestamp
    ...
    @patch('jobs.utils.s3_writer_utils.count_df')
    @patch('jobs.utils.s3_writer_utils.show_df')
    def test_write_to_s3_format_basic_flow(self, mock_show_df, mock_count_df, mock_cleanup, mock_normalise, mock_read_csv):
    ...
    def test_normalise_dataframe_schema_entity_table(self):
        ...
        with patch('jobs.utils.s3_writer_utils.load_metadata') as mock_load_metadata:
            ...
            with patch('jobs.utils.s3_writer_utils.transform_data_entity_format') as mock_transform:
                mock_transform.return_value = mock_df
                
                mock_spark = Mock()
                
                try:
                    result = normalise_dataframe_schema(mock_df, "entity", "test-dataset", mock_spark, "development")
    ...
    def test_normalise_dataframe_schema_unknown_table(self):
        ...
        with patch('jobs.utils.s3_writer_utils.load_metadata') as mock_load_metadata:
            ...
            with pytest.raises(ValueError, match="Unknown table name"):
                normalise_dataframe_schema(mock_df, "unknown_table", "test-dataset", mock_spark, "development")