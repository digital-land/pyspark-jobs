...
class TestModuleConstants:
    ...
    def test_entity_table_name_constant(self):
        ...
        assert len(ENTITY_TABLE_NAME) > 0le(self, mock_pg8000):
        """Test centroid calculation with specific target table."""
        mock_conn = Mock()
        mock_cur = Mock()
        mock_cur.rowcount = 25
        ...
        assert result == 25
        # Verify the SQL uses the target table
        sql_calls = [str(call) for call in mock_cur.execute.call_args_list]
        assert any("staging_table" in sql for sql in sql_calls)
...
class TestPrepareGeometryColumns:
    ...
    def test_prepare_geometry_columns_basic(self):
        ...
        with patch('pyspark.sql.functions.col') as mock_col, \
            ...
            assert mock_df.withColumn.call_count >= 3  # entity + geometry + point
...
class TestGetPerformanceRecommendations:
    ...
    def test_medium_dataset_recommendations(self):
        ...
        assert result["batch_size"] == 3000
        assert result["num_partitions"] == 4
        assert "Medium dataset" in result["notes"][0]
    ...
    def test_large_dataset_recommendations(self):
        ...
        assert result["batch_size"] == 4000
        assert result["num_partitions"] == 8
        assert "Large dataset" in result["notes"][0]
...
class TestWriteToPostgresOptimized:
    ...
    def test_write_optimized_auto_calculations(self, mock_prepare, mock_create):
        """Test optimized write with auto-calculated parameters."""
        mock_df = Mock()
    ...
    @patch('jobs.dbaccess.postgres_connectivity.create_table')
    @patch('jobs.dbaccess.postgres_connectivity._prepare_geometry_columns')
    def test_write_optimized_with_target_table(self, mock_prepare, mock_create):
        """Test optimized write to staging table."""
        mock_df = Mock()
        ...
        _write_to_postgres_optimized(mock_df, "test-dataset", conn_params, target_table="staging_table")
        
        # Should not call create_table for staging
        mock_create.assert_not_called()
        mock_prepare.assert_called_once()
    
    @patch('jobs.dbaccess.postgres_connectivity.create_table')
    @patch('jobs.dbaccess.postgres_connectivity._prepare_geometry_columns')
    def test_write_optimized_retry_logic(self, mock_prepare, mock_create):
        """Test retry logic in optimized write."""
        mock_df = Mock()
        mock_df.count.return_value = 1000
        mock_df.rdd.getNumPartitions.return_value = 1
        ...
        mock_write = Mock()
        mock_write.side_effect = [Exception("Connection timeout"), None]
