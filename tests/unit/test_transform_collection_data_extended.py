...
class TestTransformCollectionDataExtended:
    ...
    def test_transform_data_entity_with_mocks(self, mock_s3_typology, mock_typology, spark, sample_entity_data):
        ...
        with patch.object(spark.read, 'option') as mock_option:
            ...
            result = transform_data_entity(sample_entity_data, "test-dataset", spark, "test")
    ...
    def test_transform_data_entity_no_priority_column(self, spark):
        ...
        with patch('jobs.transform_collection_data.get_dataset_typology', return_value="geography"):
            with patch.object(spark.read, 'option') as mock_option:
                mock_org_df = spark.createDataFrame([], ["organisation", "entity", "name"])
                mock_option.return_value.csv.return_value = mock_org_df
