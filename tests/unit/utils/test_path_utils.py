...
from jobs.utils.path_utils import (
    resolve_desktop_path, resolve_repo_path, load_json_from_repo
)
...
class TestPathUtils:
    ...
    def test_resolve_desktop_path_root_file(self, mock_expanduser):
        ...
        result = resolve_desktop_path("document.pdf")
        
        expected = "/home/testuser/Desktop/document.pdf"
        assert result == expected
    ...
    def test_load_json_from_repo_unicode_content(self, mock_file, mock_resolve_path):
        ...
        mock_file.return_value.read.return_value = json.dumps(test_json, ensure_ascii=False)
...
class TestPathUtilsIntegration:
    ...
    def test_complete_json_loading_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('jobs.utils.path_utils.resolve_repo_path', return_value=json_file):
    ...
    def test_error_handling_workflow(self):
        ...
        # Test file not found
        with patch('jobs.utils.path_utils.resolve_repo_path', return_value="/nonexistent/path.json"):
            with pytest.raises(FileNotFoundError):
                load_json_from_repo("nonexistent.json")
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as temp_file:
            ...
            try:
                with patch('jobs.utils.path_utils.resolve_repo_path', return_value=temp_file.name):
                    with pytest.raises(json.JSONDecodeError):
                        load_json_from_repo("invalid.json")
            finally:
