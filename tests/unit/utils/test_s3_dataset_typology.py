...
from jobs.utils.s3_dataset_typology import (
    DATASET_SPEC_URL, load_datasets, get_dataset_typology
)
...
listed-building,heritage,Listed building"""
        ...
        mock_response = Mock()
        mock_response.readlines.return_value = [line.encode('utf-8') for line in csv_content.split('\n')]
...
encoding-test,geography,Test with émojis 🏠"""
    ...
    def test_get_dataset_typology_found(self, mock_load_datasets):
        ...
        mock_datasets = [
            {'dataset': 'transport-access-node', 'typology': 'geography', 'name': 'Transport access node'},
            {'dataset': 'listed-building', 'typology': 'heritage', 'name': 'Listed building'},
            {'dataset': 'conservation-area', 'typology': 'geography', 'name': 'Conservation area'}
        ]
    ...
    def test_get_dataset_typology_missing_typology_field(self, mock_load_datasets):
        ...
        mock_datasets = [
            {'dataset': 'transport-access-node', 'name': 'Transport access node'},  # Missing typology
            {'dataset': 'listed-building', 'typology': 'heritage', 'name': 'Listed building'}
        ]
        mock_load_datasets.return_value = mock_datasets
    ...
    def test_get_dataset_typology_empty_typology_value(self, mock_load_datasets):
        ...
        mock_datasets = [
            {'dataset': 'transport-access-node', 'typology': '', 'name': 'Transport access node'},
            {'dataset': 'listed-building', 'typology': 'heritage', 'name': 'Listed building'}
        ]
        mock_load_datasets.return_value = mock_datasets
    ...
    def test_get_dataset_typology_multiple_matches(self, mock_load_datasets):
        ...
        mock_datasets = [
            {'dataset': 'duplicate-dataset', 'typology': 'first-typology', 'name': 'First Dataset'},
            {'dataset': 'duplicate-dataset', 'typology': 'second-typology', 'name': 'Second Dataset'},
            {'dataset': 'other-dataset', 'typology': 'other-typology', 'name': 'Other Dataset'}
        ]
        mock_load_datasets.return_value = mock_datasets
    ...
    def test_get_dataset_typology_none_input(self):
        ...
        with patch('jobs.utils.s3_dataset_typology.load_datasets') as mock_load_datasets:
            mock_datasets = [
                {'dataset': 'test-dataset', 'typology': 'test-typology', 'name': 'Test Dataset'}
            ]
            mock_load_datasets.return_value = mock_datasets
...
extra-field,geography,Extra Field Dataset,Normal dataset,live,collection,extra-consideration,extra-field'''
    ...
    def test_performance_with_large_dataset_simulation(self):
        ...
        with patch('jobs.utils.s3_dataset_typology.load_datasets', return_value=large_datasets):
