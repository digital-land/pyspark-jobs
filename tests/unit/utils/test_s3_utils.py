...
from jobs.utils.s3_utils import (
    S3UtilsError, parse_s3_path, validate_s3_path, cleanup_dataset_data,
    validate_s3_bucket_access, _extract_bucket_name_safe, read_csv_from_s3
)
...
class TestS3Utils:
    ...
    def test_cleanup_dataset_data_success(self, mock_boto3):
        ...
        mock_paginator.paginate.assert_called_once_with(
            Bucket='bucket', 
            Prefix='prefix/dataset=test-dataset/'
        )
    ...
    def test_cleanup_dataset_data_batch_deletion(self, mock_boto3):
        ...
        objects = [{'Key': f'prefix/dataset=test-dataset/file{i}.parquet'} for i in range(1500)]
...
class TestS3UtilsIntegration:
    ...
    def test_complete_cleanup_workflow(self, mock_boto3):
        ...
        mock_paginator.paginate.assert_called_once_with(
            Bucket='test-bucket',
            Prefix='data/dataset=test-dataset/'
        )
        assert mock_s3.delete_objects.call_count == 2
