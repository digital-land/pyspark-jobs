...
with patch.dict('sys.modules', {
    ...
    from jobs.main_collection_data import (
        initialize_logging, create_spark_session, load_metadata,
        read_data, transform_data, validate_s3_path, main
    )
...
class TestMainCollectionData:
    ...
    def test_load_metadata_local_file(self, mock_abspath, mock_dirname, mock_isabs, mock_open):
    ...
    def test_main_missing_arguments(self):
        ...
        args = Mock()
        args.env = "development"  # Add env to avoid AttributeError in initialize_logging
    ...
    def test_main_full_load_success(self, spark, sample_fact_data):
        ...
        assert mock_transform.call_count >= 3  # Called for fact, fact_resource, entity tables
    ...
    @patch('jobs.main_collection_data.initialize_logging')
    @patch('jobs.main_collection_data.create_spark_session')
    def test_main_timing_calculation(self, mock_spark_session, mock_init_logging, mock_datetime):
        ...
        with patch('jobs.main_collection_data.read_data', side_effect=Exception("Test error")):
            try:
                main(args)
            except Exception:
                pass
