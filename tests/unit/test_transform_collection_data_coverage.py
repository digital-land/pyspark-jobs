...
class TestTransformDataFactCoverage:
    ...
    def test_transform_data_fact_column_selection(self):
        ...
        expected_first_select = ["fact", "end_date", "entity", "field", "entry_date", 
                               "priority", "reference_entity", "start_date", "value"]
        expected_final_select = ["end_date", "entity", "fact", "field", "entry_date", 
                               "priority", "reference_entity", "start_date", "value"]
        
        calls = mock_selected_df.select.call_args_list
        assert len(calls) == 2
        assert calls[0][0] == tuple(expected_first_select)
        assert calls[1][0] == tuple(expected_final_select)
...
class TestTransformDataFactResCoverage:
    ...
    def test_transform_data_fact_res_column_operations(self):
        ...
        expected_first_select = ["end_date", "fact", "entry_date", "entry_number", 
                               "priority", "resource", "start_date"]
        expected_final_select = ["end_date", "fact", "entry_date", "entry_number", 
                               "priority", "resource", "start_date"]
...
class TestTransformDataEntityCoverage:
    ...
    def test_transform_data_entity_priority_column_check(self, mock_show_df, mock_typology):
        ...
        mock_ranked_df.groupBy.return_value.pivot.return_value.agg.return_value = mock_pivot_df
        ...
        mock_pivot_df.join.return_value.select.return_value.drop.return_value = mock_pivot_df
        ...
        result = transform_data_entity(mock_df_with_priority, "test-dataset", mock_spark, "development")
    ...
    @patch('jobs.transform_collection_data.get_dataset_typology')
    @patch('jobs.transform_collection_data.show_df')
    def test_transform_data_entity_without_priority_column(self, mock_show_df, mock_typology):
        ...
        result = transform_data_entity(mock_df_no_priority, "test-dataset", mock_spark, "development")
    ...
    @patch('jobs.transform_collection_data.get_dataset_typology')
    @patch('jobs.transform_collection_data.show_df')
    def test_transform_data_entity_column_normalization(self, mock_show_df, mock_typology):
    ...
    @patch('jobs.transform_collection_data.get_dataset_typology')
    @patch('jobs.transform_collection_data.show_df')
    def test_transform_data_entity_missing_columns_handling(self, mock_show_df, mock_typology):
        ...
        expected_missing_columns = ["geometry", "end_date", "start_date", "name", "point"]
        
        # Check that some missing columns are added
        missing_columns_added = any(col in added_columns for col in expected_missing_columns)
        assert missing_columns_added or len(withColumn_calls) >= 5

    @patch('jobs.transform_collection_data.get_dataset_typology')
    @patch('jobs.transform_collection_data.show_df')
    def test_transform_data_entity_json_creation_logic(self, mock_show_df, mock_typology):
    ...
    @patch('jobs.transform_collection_data.get_dataset_typology')
    @patch('jobs.transform_collection_data.show_df')
    def test_transform_data_entity_date_normalization(self, mock_show_df, mock_typology):
        ...
        date_columns_processed = any(
            "date" in str(call) for call in withColumn_calls
        )
        assert date_columns_processed or len(withColumn_calls) >= 3

    @patch('jobs.transform_collection_data.get_dataset_typology')
    @patch('jobs.transform_collection_data.show_df')
    def test_transform_data_entity_geometry_normalization(self, mock_show_df, mock_typology):
        ...
        geometry_columns_processed = any(
            "geometry" in str(call) or "point" in str(call) 
    ...
    def test_transform_data_entity_final_projection(self, mock_show_df, mock_typology):
        ...
        result = transform_data_entity(mock_df, "test-dataset", mock_spark, "development")
